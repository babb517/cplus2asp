...
bool hack2 = true;//use the hack to deal with terminal comments in included files
					...
					if((curTokIter != ltsyyLexer::streamEnd()) && (((*curTokIter)->tokenID == T_PERIOD) || ((*curTokIter)->stringValue).compare(".") == 0))
					{
						...
						for(includeListIterator = includeList.begin(); includeListIterator != includeList.end(); includeListIterator++)//add each files tokens to the current stream.
						{
							...
							if(flexin != NULL)							
							{
								...
								if(hack2)
								{								
									//hack: put a NOOP at the end of the file
									std::ofstream tempfile ((*includeListIterator).c_str(), std::ios::app);
									if (tempfile.is_open())
									{
										tempfile << "NOOP\n";
										tempfile.close();
								...
								if(hack2)
								{
									//hack: take the NOOP back off
									std::ofstream newfile ("temp");
									std::ifstream tempfile2 ((*includeListIterator).c_str());
									if (tempfile2.is_open())
									{
										...
										tempfile2.close();
										newfile.close();
										remove((*includeListIterator).c_str());
										rename("temp",(*includeListIterator).c_str());
									}
									else std::cout << "Unable to open file";
									//hack
								}
