/* Generated by re2c 0.13.5 on Sun Apr 28 11:52:15 2013 */
#line 1 "Config.re2c"
/*
...
Config::Config(std::string const& name) {
	...
	mQueries[CONST_QUERY_NONE] = Query("Satisfiability Check", CONST_QUERY_NONE, UNDEFINED, UNDEFINED);
}
...
void Config::compileArgs(Toolchain tool, std::list<std::string>& arglist, RunConfig const* subconfig)  const {
	...
	case TC_SOLVER:
		...
		if (!run(TC_GROUNDER)) {
			if (subconfig) {
				if (subconfig->query != "") arglist.push_back("-c query=" + query(subconfig->query)->cmd);
	...
	case TC_GROUNDER:
		...
		if (subconfig) {
			if (subconfig->query != "") arglist.push_back("-c query=" + utils::to_string(subconfig->query));
			if (subconfig->maxstep != UNDEFINED) arglist.push_back("-c maxstep=" + utils::to_string(subconfig->maxstep));
			if (subconfig->minstep != UNDEFINED) arglist.push_back("-c minstep=" + utils::to_string(subconfig->minstep));
		} else {
