/* Generated by re2c 0.13.5 on Sun Mar 31 01:04:05 2013 */
#line 1 "Config.re2c"
/* 
...
// Install directory
std::string Config::installDir() const {
	char tmp[PATH_MAX];
...
std::string Config::compileCommandLine(Toolchain tool, RunConfig const* subconfig) const {
	...
	case TC_SOLVER:
		...
		if (!run(TC_GROUNDER)) {
			if (subconfig) {
				if (subconfig->queryId != UNDEFINED) out << " -c query=" << subconfig->queryId;
				if (subconfig->maxstep != UNDEFINED) out << " -c maxstep=" << subconfig->maxstep;
				if (subconfig->minstep != UNDEFINED) out << " -c minstep=" << subconfig->minstep;
			} else {
		...
		if (mode() == MODE_INCREMENTAL || mode() == MODE_REACTIVE) {
			if (subconfig) {
				if (subconfig->minstep != UNDEFINED) out << " --imin=" << subconfig->minstep;
				if (subconfig->maxstep != UNDEFINED) {
					if (subconfig->maxstep > 0)
						out << " --imax=" << subconfig->maxstep;
					else
						out << " --ibase";
				}
			} else {
		...
		if (mode() == MODE_REACTIVE && run(TC_REACTIVE_BRIDGE)) {
			...
			out << " --port=" << intConfigOpt(OPT_EXT_PORT);
#else
			out << " --port=" << intConfigOpt(OPT_INT_PORT);
#endif
	...
	case TC_REACTIVE_BRIDGE:
		...
		if (boolConfigOpt(OPT_NONE_HACK)) out << " --none=" << HACKED_NONE_VALUE;
#endif
		break;
...
yy88:
		++ext;
#line 809 "Config.re2c"
		{ out.tool = TC_TRANSLATOR; out.runningMode = MODE_STATIC_AUTO | MODE_STATIC_MANUAL | MODE_INCREMENTAL; return out;}
#line 1231 "<stdout>"
	}
#line 839 "Config.re2c"
