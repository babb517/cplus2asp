/* Generated by re2c 0.13.5 on Tue Apr 23 00:50:57 2013 */
#line 1 "Config.re2c"
/*
...
void Config::compileArgs(Toolchain tool, std::list<std::string>& arglist, RunConfig const* subconfig)  const {
	...
	case TC_SOLVER:
		...
		if (!run(TC_GROUNDER)) {
			if (subconfig) {
				if (subconfig->queryId != UNDEFINED) arglist.push_back("-c query=" + query(subconfig->queryId)->cmd);
...
bool Config::addQuery(unsigned int queryID, std::string const& name, std::string const& cmd, unsigned int maxstep, unsigned int minstep) {
	...
	if (hasQuery(queryID) || hasQuery(cmd)) unique = false;
...
bool Config::addQuery(Query const& query) {
	...
	// ensure that the query is not already present...
	if (hasQuery(query.id) || hasQuery(query.cmd)) unique = false;
...
// Checks if the query exists by command.
bool Config::hasQuery(std::string const& cmd) const {
	for(std::pair<unsigned int,Query> const& q : mQueries) {
		if (q.second.cmd == cmd) return true;
	}
	return false;
}
