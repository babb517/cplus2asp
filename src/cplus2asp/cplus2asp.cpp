/* Generated by re2c 0.13.5 on Wed Apr 24 22:59:22 2013 */
...
/* History:
 * v3.2 - Major revision of command-line arguments. Revised command line calls to provide system portability.
	...
	if ((intSystemResult = runTool(config, Config::TC_TRANSLATOR, "translator")) != TC_STAT_TOOLSKIPPED) {
		...
		if (!config.run(Config::TC_PREPROC)) {
			if (!echoFile(std::cout, config.output(Config::TC_TRANSLATOR))) {
				error("Unable to read from the translator output file '%s'", true, -1, config.output(Config::TC_TRANSLATOR).c_str());
			}
		}
	...
	if ((intSystemResult = runTool(config, Config::TC_PREPROC, "pre-processor")) != TC_STAT_TOOLSKIPPED)  {
		...
		if (!config.run(Config::TC_GROUNDER) && !config.run(Config::TC_SOLVER)) {
			if (!echoFile(std::cout, config.output(Config::TC_PREPROC))) {
				error("Unable to read from the pre-processor output file '%s'", true, -1, config.output(Config::TC_PREPROC).c_str());
			}
		}
			...
			if ((intSystemResult = runTool(config, Config::TC_GROUNDER, "grounder", &runConfig)) != TC_STAT_TOOLSKIPPED)  {
				...
				if (!config.run(Config::TC_SOLVER)) {
					if (!echoFile(std::cout, config.output(Config::TC_GROUNDER))) {
						error("Unable to read from the grounder output file '%s'", true, -1, config.output(Config::TC_GROUNDER).c_str());
					}
				}
			...
			if ((intSystemResult = runTool(config, Config::TC_SOLVER, "solver", &runConfig)) != TC_STAT_TOOLSKIPPED)  {
				...
				if (!config.run(Config::TC_POSTPROC)) {
					if (!echoFile(std::cout, config.output(Config::TC_SOLVER))) {
						error("Unable to read from the solver output file '%s'", true, -1, config.output(Config::TC_SOLVER).c_str());
					}
				}
			...
			if (config.mode() != Config::MODE_REACTIVE) {
				if ((intSystemResult = runTool(config, Config::TC_POSTPROC, "post-processor", &runConfig)) != TC_STAT_TOOLSKIPPED)  {
					if (!echoFile(std::cout, config.output(Config::TC_POSTPROC))) {
						error("Unable to read from the post-processor output file '%s'", true, -1, config.output(Config::TC_POSTPROC).c_str());
					}
					// It Ran!
	...
	for(int i = 1; i < argc; i++)
	{
		...
		case OPT_BRIDGE:
			...
			if (outConf.command(Config::TC_REACTIVE_BRIDGE, tmpptr)) {
				warning("Detected redefinition of bridge command. Continuing with value '%s'",true,tmpptr);
			}
		...
		case OPT_BRIDGE_OPTS:
			...
			if (!tmpptr) {
				if (++i < argc) {
					...
					// setup for a command but they didn't deliver.
					error("Expected pass-through options following option '%s'. Exiting.",true,0,argv[i-1]);
					return OPT_STAT_BAD_ARG;
				}
			}

			if (outConf.opts(Config::TC_REACTIVE_BRIDGE, tmpptr)) {
				warning("Detected redefinition of bridge command options. Continuing with value '%s'",true,tmpptr);
			}
			break;
		case OPT_BRIDGE_OUT:

			error("Redirecting the reactive bridge's output is currently unsupported.", true, 0);
			return OPT_STAT_BAD_ARG;
		...
		case OPT_GRD:
			...
			if (outConf.command(Config::TC_GROUNDER, tmpptr)) {
				warning("Detected redefinition of grounder command. Continuing with value '%s'",true,tmpptr);
			}
		...
		case OPT_GRD_OPTS:
			...
			if (outConf.opts(Config::TC_GROUNDER, tmpptr)) {
				warning("Detected redefinition of grounder command options. Continuing with value '%s'",true, tmpptr);
			}
		...
		case OPT_GRD_OUT:
			...
			if (!tmpptr) {
				if (++i < argc) {
					...
					// setup for a command but they didn't deliver.
					error("Expected a file name following option '%s'. Exiting.",true,0,argv[i-1]);
					return OPT_STAT_BAD_ARG;
				}
			}

			if (outConf.output(Config::TC_GROUNDER, tmpptr)) {
				warning("Detected redefinition of the grounder output file. Continuing with value '%s'",true, tmpptr);
			}
			break;
		...
		case OPT_GRINGO_NONE_HACK:
			...
			if (!tmpptr) {
				warning("Option '%s' has been deprecated in favor of specifying an explicit none alias value.",true,argv[i]);

				tmpptr = DEF_NONE_HACK_VAL;
			}


			if (outConf.noneAlias(tmpptr)) {
				warning("Detected redefinition of the none alias option. Continuing with value '%s'",true, tmpptr);
			}
			...
			if (!tmpptr) {
				if (++i < argc) {
					tmpptr = argv[i];
						...
						error("Expected a language specification following option '%s'. Exiting.",true,0, argv[i-1]);
						return OPT_STAT_BAD_ARG;
				}
			}

			if (!outConf.parseLang(tmpptr, l)) {
				error("'%s' is not a supported language specification.", 0, true, tmpptr);
				return OPT_STAT_BAD_ARG;
			}

			if (outConf.lang(l)) {
				warning("Detected a redefition of the input language.",true,argv[i]);
			}
			break;
		...
		case OPT_MAXSTEP:
			...
			if ((steps = Config::parseMaxstep(tmpptr)).second == Config::UNDEFINED) {
				error("'%s' is not a valid step value. Exiting.",true,0, tmpptr);
				return OPT_STAT_BAD_ARG;
			...
			{
				warning("Detected a redefinition of the maximum step. Continuing with the value '%s'.",true,tmpptr);
			}
			break;
		...
		case OPT_MINSTEP:
			...
			if (!from_string(tmpInt, tmpptr)) {
				error("'%s' is not a valid step value. Exiting.",true,0, tmpptr);
				return OPT_STAT_BAD_ARG;
			} else if (outConf.intConfigOpt(Config::OPT_MINSTEP, tmpInt)) {
				warning("Detected a redefinition of the minimum step. Continuing with value '%s'.",true,tmpptr);
			}
		...
		case OPT_MODE_SELECT:
			...
			if (!strcmp(tmpptr,ARG_MODE_STATIC_MANUAL)) {
				...
				error("Unexpected value '%s'. Expected a mode option (one of '%s', '%s', '%s', or '%s'). Exiting.",true,0,
						tmpptr, ARG_MODE_STATIC_AUTO, ARG_MODE_STATIC_MANUAL, ARG_MODE_INCREMENTAL, OPT_MODE_REACTIVE);
				return OPT_STAT_BAD_ARG;
			}

			if (outConf.mode(mode)) {
				warning("Detected a redefinition of the program's running mode. Continuing with value '%s.", true, tmpptr);
			}
		...
		case OPT_NO_DEL:
			tmpBool = false;
			warning("The option '%s' has been deprecated in favor of the '--[PROG]-output=[OUTFILE] option.", true, argv[i]);
		...
		case OPT_SHIFT:
			...
			if (!strcmp(tmpptr, "true")) {
				...
				error("Invalid shift value. Expected a Boolean value ('true' or 'false'). Exiting.", true, 0, argv[i]);
				return OPT_STAT_BAD_ARG;
			}
		...
		case OPT_PORT_EXTERNAL:
			...
			if (!from_string(tmpInt, tmpptr) || tmpInt < 1) {
				error("'%s' is not a valid port. Exiting.",true,0, tmpptr);
				return OPT_STAT_BAD_ARG;
			} else if (outConf.intConfigOpt(Config::OPT_EXT_PORT, tmpInt)) {
				warning("Detected a redefinition of the external port. Continuing with value '%s'.",true,tmpptr);
			}
		...
		case OPT_PORT_INTERNAL:
			...
			if (!from_string(tmpInt, tmpptr) || tmpInt < 1) {
				...
				warning("Detected a redefinition of the internal port. Continuing with value '%s'.",true,tmpptr);
			}
		...
		case OPT_POSTPROC_OPTS:
			...
			if (outConf.opts(Config::TC_POSTPROC, tmpptr)) {
				warning("Detected redefinition of post-processor command options. Continuing with value '%s'",true,tmpptr);
			}
		...
		case OPT_POSTPROC_OUT:
			...
			if (outConf.output(Config::TC_POSTPROC, tmpptr)) {
				warning("Detected redefinition of the post-processor output file. Continuing with value '%s'",true, tmpptr);
			}
			break;
		...
		case OPT_PREPROC:
			...
			if (outConf.command(Config::TC_PREPROC, tmpptr)) {
				warning("Detected redefinition of pre-processor command. Continuing with value '%s'",true,tmpptr);
			}
			break;
		...
		case OPT_PREPROC_OPTS:
			...
			if (outConf.opts(Config::TC_PREPROC, tmpptr)) {
				warning("Detected redefinition of pre-processor command options. Continuing with value '%s'",true,tmpptr);
			}
		...
		case OPT_PREPROC_OUT:
			...
			if (outConf.output(Config::TC_PREPROC, tmpptr)) {
				warning("Detected redefinition of the pre-processor output file. Continuing with value '%s'",true, tmpptr);
			}
			break;
		...
		case OPT_SO:
			...
			if (outConf.command(Config::TC_SOLVER, tmpptr)) {
				warning("Detected redefinition of solver command. Continuing with value '%s'",true,tmpptr);
			}
			break;
		...
		case OPT_SO_OPTS:
			...
			if (outConf.opts(Config::TC_SOLVER, tmpptr)) {
				warning("Detected redefinition of solver command options. Continuing with value '%s'",true, tmpptr);
			}
		...
		case OPT_SO_OUT:
			...
			if (outConf.output(Config::TC_SOLVER, tmpptr)) {
				warning("Detected redefinition of the solver output file. Continuing with value '%s'",true, tmpptr);
			}
			break;
		...
		case OPT_SYMTAB:
			...
			if (outConf.symTabFile(tmpptr)) {
				warning("Detected redefinition of the symbol table file. Continuing with value '%s'",true, tmpptr);
			}
			break;
		...
		case OPT_TRANS:
			...
			if (outConf.command(Config::TC_TRANSLATOR, tmpptr)) {
				warning("Detected redefinition of translator command. Continuing with value '%s'.",true,tmpptr);
			}
		...
		case OPT_TRANS_OPTS:
			...
			if (outConf.opts(Config::TC_TRANSLATOR, tmpptr)) {
				warning("Detected redefinition of translator command options. Continuing with value '%s'.",true,tmpptr);
			}
		...
		case OPT_TRANS_OUT:
			...
			if (outConf.output(Config::TC_TRANSLATOR, tmpptr)) {
				error("Unable to open '%s' for reading/writing.",true,0,tmpptr);
				return OPT_STAT_BAD_ARG;
			} else if (tmpInt > 0) {
				warning("Detected redefinition of the translator output file. Continuing with value '%s'",true, tmpptr);
			}
			break;
...
yy193:
		++opt;
#line 1792 "cplus2asp.re2c"
		{ return OPT_UNSUPPORTED; }
#line 2645 "<stdout>"
yy195:
		++opt;
#line 1791 "cplus2asp.re2c"
		{ return OPT_UNSUPPORTED; }
#line 2650 "<stdout>"
yy197:
		yych = *++opt;
...
std::ostream& showHelp(std::string const& execName, std::ostream& out) {
	out   << "Usage: " << execName << " <INPUT_FILES> [<OPTIONS>] [<CONSTANTS>] [<NUM_SOLUTIONS>]" << endl
		  ...
		  << endl
		  << "  <NUM_SOLUTIONS> ------- The desired number of solutions for the solver to find, or 'all'." << endl
		  ...
		  << endl
		  << "  <CONST>=<VALUE>, -c <CONST>=<VALUE>" << endl
		  ...
		  << endl
		  << " -q <QUERY>, -q=<QUERY>, --query <QUERY>, query=<QUERY>"  << endl
		  ...
		  << endl
		  << " -max <STEP>, -max=<STEP>, --maxstep <STEP>, maxstep=<STEP>" << endl
		  ...
		  << endl
		  << " -min <STEP>, -min=<STEP>, --minstep <STEP>, minstep=<STEP>" << endl
...
// TODO: Uncomment this when we have the reactive bridge complete w/ remote program support.
...
#if 0
		  << " -p <PORT]" << endl
		  << " --port [PORT] ---------- Sets the port used to connect with external client programs instead " << endl
		  << "                          of using the console to gather online input. This option is only " << endl
		  << "                          valid in reactive mode." << endl
#endif
		  << " -i <PORT>, -i=<PORT>, --internal-port <PORT>" << endl
		  ...
		  << "                          reactive mode." << endl
#else
		  << " -p <PORT>, -p=<PORT>, --port <PORT>" << endl
		  << " --port=<PORT> ---------- Sets the port used to connect with external client programs." << endl
#endif
		  << endl
		  << " -na[=<VALUE>]" << endl
		  << " --none-alias[=<VALUE>]   Indicates that we should simulate 'none' with <VALUE> [default: '" << DEF_NONE_HACK_VAL << "']" << endl
		  << "                          This is helpful for circumventing a bug in Gringo 3.0.4 that " << endl
		  << "                          sometimes prevents domains from properly grounding due to " << endl
		  << "                          unification issues." << endl
		  << endl
		  << " -s[=<BOOLEAN>]" << endl
		  << " --shift[=<BOOLEAN>] ---- Sets whether to allow the solver to shift disjunction in the head of a rule " << endl
		  << "                          generated by the pre-processor into the body. This is useful when " << endl
		  << "                          the preprocessor generate disjunctive rules and a disjunctive " << endl
		  << "                          solver isn't available. May not be guaranteed to preserve stable " << endl
		  << "                          models." << endl
		  << endl
		  << " --stats ---------------- Indicates additional solving statistics should be requested from" << endl
		  << "                          the solver and displayed after each solver run." << endl
		  << endl
		  << " --language=<LANG> ------ Sets the input language that will be accepted for translation/running." << endl
		  << "                          <LANG> can be one of the following:" << endl
		  << "                          * c+, C+ -------- [default] The action language C+." << endl
 		  << "                          * bc, BC -------- The action language BC." << endl
		  << endl
		  ...
		  << endl
		  << "  -<PROG> <PATH>, -<PROG>=<PATH>, --<PROGRAM> <PATH> " << endl
		  << "  --<PROGRAM>=<PATH> ------ Gives a new path to the desired tool chain program." << endl
		  << endl
		  << "  -<PROG>opt <OPTS>, -<PROG>opt=<OPTS>, --<PROGRAM>-opts <OPTS>" << endl
		  << "  --<PROGRAM>-opts=<OPTS> -- Passes <OPTS> to the given tool chain program." << endl
		  << endl
		  << "  -<PROG>out <FILE>, -<PROG>out=<FILE>, --<PROGRAM>-output <FILE>" << endl
		  << "  --[PROGRAM]-output=<FILE> Write the output of the specified tool to <FILE>." << endl
		  << endl
		  << "  -from<PROG> " << endl
		  ...
		  << endl
		  << " -symtab <FILE>, -symtab=<FILE>, --symbol-table=<FILE> " << endl
		  << " --symbol-table=<FILE> -- Writes the program's symbol table to <FILE> for use in other programs." << endl
		  << endl
				...
				&& tempPos2 > tempPos)
		{
			...
			if (foundQuery) {
				...
				if (!config.addQuery(tempQuery)) {
					warning("[%s:%d]: Detected duplicate queries with ID '%s'. Ignoring the most recent one found.", file.c_str(), currentLine, tempQuery.cmd.c_str());
				}
			}
		}
	}
...
//		std::string tmpCmd = config.compileCommandLine(tool, subconfig);
		...
		debug("Running the '%s' (program: '%s') with command: '%s'",true,name.c_str(), config.command(tool).c_str(), config.compileCommandLine(tool, subconfig).c_str());
		...
		if (!fsink.is_open()) {
			if (config.customOut(tool))
				error("Unable to open '%s' for writing.", true, 0, config.output(tool).c_str());
			else
				error("Unable to open temporary file '%s' for writing.", true, 0, config.output(tool).c_str());
		...
		if (!errsink.is_open()) {
			error("Unable to open temporary file '%s' for writing.", true, 0, config.errFile().c_str());
			return TC_STAT_IO_ERR;
		}
...
//				bp::initializers::start_in_dir(boost::filesystem::current_path().string())
		...
		debug("Command exited with status '%d'.", true, ret);

		if (!processErrors(config, tool, config.errFile())) {
			warning("Could not read from the error stream file.", true);
		}
...
bool processErrors(Config const& config, Config::Toolchain tool, std::string const& file) {
	...
	if (unificationError) {
		error("Detected an error that appears to be caused by a bug in Gringo 3.0.5 which prevents", true, 0);
		error("  certain programs from being grounded properly. Adding '--none-alias[=VALUE]' to", true, 0);
		error("  the command-line arguments may fix this problem.", true, 0); 
	}
...
void cleanup(Config const& config, bool const* ran) {
	debug("Deleting appropriate temporary files.", true);
	...
	for (int i = 0; i < (int)Config::TC_LENGTH; i++) {
		if (!config.customOut((Config::Toolchain)i)) {
			debug("Deleting '%s'.", true, config.output((Config::Toolchain)i).c_str());
