/* Generated by re2c 0.13.5 on Fri Feb 22 17:05:58 2013 */
...
int main(int argc, char** argv)
{
	...
	do {
		...
		runConfig.minstep = (config.intConfigOpt(Config::OPT_MINSTEP) != Config::UNDEFINED)
					? config.intConfigOpt(Config::OPT_MINSTEP)
					: 0;
...
yy24:
#line 1450 "cplus2asp.re2c"
		{ return OPT_SHIFT_TRUE; }
#line 1587 "<stdout>"
yy25:
		++opt;
#line 1435 "cplus2asp.re2c"
...
yy103:
		++opt;
#line 1438 "cplus2asp.re2c"
		{ return OPT_JUST_POST; }
#line 1960 "<stdout>"
yy105:
		++opt;
#line 1437 "cplus2asp.re2c"
		{ return OPT_JUST_PRE; }
#line 1965 "<stdout>"
yy107:
		++opt;
#line 1439 "cplus2asp.re2c"
		{ return OPT_JUST_SO; }
#line 1970 "<stdout>"
yy109:
		yych = *++opt;
...
yy123:
		++opt;
#line 1447 "cplus2asp.re2c"
		{ return OPT_MINSTEP; }
#line 2037 "<stdout>"
yy125:
		++opt;
#line 1446 "cplus2asp.re2c"
		{ return OPT_MAXSTEP; }
#line 2042 "<stdout>"
yy127:
		++opt;
#line 1431 "cplus2asp.re2c"
...
bool queryUserForQuery(Config const& config, Config::RunConfig& runconfig)
{
	...
	while (!goodInput) {
		...
		// Get user input
		query = NULL;
		numSoln = config.intConfigOpt(Config::OPT_NUM_SOLN);

		std::cout << "Please select a query to run.\n";

		if (config.run(Config::TC_SOLVER))
			std::cout << "You may also optionally follow your selection with ':#' where # is the number of solutions to find (or 0 to find all solutions) [default: " << numSoln << "].\n";
		...
		if (line == EXIT_CMD) {
			// crap, they do. Return false to indicate this.
			return false;
		} else {
			if ( config.run(Config::TC_SOLVER) && (pos = line.find_first_of(':')) != std::string::npos) {
				// They've specified an answer set count.
				tok = trimWhitespace(line.substr(0, pos));
			} else {
				// no answer set count
			...
			for (Config::QueryMap::const_iterator it = config.beginQueries(); it != config.endQueries() && !query; it++) {
				if (it->second.cmd == tok) {
					query = &(it->second);
				}
			}

			if (query && config.run(Config::TC_SOLVER) && pos != std::string::npos) {
				goodInput = from_string(numSoln, trimWhitespace(line.substr(pos+1)));
			} else if (query) {
				goodInput = true;
			}
